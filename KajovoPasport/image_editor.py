...
class ImageEditor(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(controls, text="Posun: táhněte myší | Zoom: kolečko | Rotace: tlačítky").grid(row=0, column=0, columnspan=10, sticky="w")

        ttk.Button(controls, text="⟲ -90°", command=lambda: self._rotate(-90)).grid(row=1, column=0, padx=2)
        ttk.Button(controls, text="⟳ +90°", command=lambda: self._rotate(+90)).grid(row=1, column=1, padx=2)
        ttk.Button(controls, text="⟲ -5°", command=lambda: self._rotate(-5)).grid(row=1, column=2, padx=2)
        ttk.Button(controls, text="⟳ +5°", command=lambda: self._rotate(+5)).grid(row=1, column=3, padx=2)
        ttk.Button(controls, text="Reset", command=self._reset).grid(row=1, column=4, padx=8)

        ttk.Button(controls, text="Uložit", command=self._on_save).grid(row=1, column=8, padx=2)
        ttk.Button(controls, text="Zrušit", command=self._on_cancel).grid(row=1, column=9, padx=2)
